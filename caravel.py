...
class CaravelConfig():
    ...
    def instantiate(self):
        ...
        body = [
            "",
            "wire sc_clk_out, sc_data_out, sc_latch_io0_out, sc_latch_io7_1_out, sc_scan_out;",
            ...
            "   .scan_latch_io0_en      (sc_latch_io0_out),",
            "   .scan_latch_io7_1_en    (sc_latch_io7_1_out),",
            "",
            "   .la_scan_clk_in         (la_data_in[0]),",
            "   .la_scan_data_in        (la_data_in[1]),",
            "   .la_scan_data_out       (la_data_out[0]),",
            "   .la_scan_select         (la_data_in[2]),",
            "   .la_scan_latch_io0_en   (la_data_in[3]),",
            "   .la_scan_latch_io7_1_en (la_data_in[4]),",
            "",
        ...
        for idx in range(self.num_projects):
            ...
            body += [
                "",
                f"// [{idx:03d}] {giturl}",
                f"wire {pfx}_clk_out, {pfx}_data_out, {pfx}_scan_out, {pfx}_latch_io0_out, {pfx}_latch_io7_1_out;",
                ...
                f"    .latch_io0_enable_in ({prev_pfx}_latch_io0_out),",
                f"    .latch_io7_1_enable_in ({prev_pfx}_latch_io7_1_out),",
                f"    .clk_out         ({pfx}_clk_out),",
                f"    .data_out        ({pfx}_data_out),",
                f"    .scan_select_out ({pfx}_scan_out),",
                f"    .latch_io0_enable_out({pfx}_latch_io0_out),",
                f"    .latch_io7_1_enable_out({pfx}_latch_io7_1_out),",
